...
inline static sspipe_t* new_sspipe(int id) {
    ...
    pipe->resp_buf = ssbuff_init(0);
    if (pipe->resp_buf == NULL) {
        _LOG_E("new_pipe: ssbuff_init failed");
        free_sspipe(pipe);
        return NULL;
    }
...
//                                (const unsigned char*)pipe->in_buf->buf + (pipe->in_buf->len - remaining),
//                                payload_len, (unsigned char*)ctx->pkt_buf + SSP_PACKET_HEAD_LEN, &cipher_len) !=
//                                0) {
